def getInmFREDBaseImg(fileName, dtype=float, displayInfo=False):
    ...
    with open(fileName, "rb") as file_h:
        ...
        size = np.prod(shape)
        spacing = np.around(np.array([spacingX, spacingY, spacingZ]), decimals=6) * 10
        offset = np.around(np.array([offsetX, offsetY, offsetZ]), decimals=6) * 10
...
def getInmFREDSumImage(fileName, inmInfo=None, threshold=None, dtype=float, displayInfo=False):
    ...
    with open(fileName, "rb") as file_h:
        ...
        shape = np.array([sizeX, sizeY, sizeZ])
        size = np.prod(shape)
        spacing = np.around(np.array([spacingX, spacingY, spacingZ]), decimals=6) * 10  # [mm]
        offset = np.around(np.array([offsetX, offsetY, offsetZ]), decimals=6) * 10  # [mm]
...
def getInmFREDPoint(fileName, point, inmInfo=None, dtype=float, interpolation="linear", raiseMemError=True, displayInfo=False):
    ...
    with open(fileName, "rb") as file_h:
        ...
        shape = np.array([sizeX, sizeY, sizeZ])
        size = np.prod(shape)
        spacing = np.around(np.array([spacingX, spacingY, spacingZ]) * 10, decimals=6)  # [mm]
        offset = np.around(np.array([offsetX, offsetY, offsetZ]) * 10, decimals=6)  # [mm]
        origin = offset + spacing / 2  # [mm]
